-- Enhanced Inventory Integration Migration (Fixed)
...
BEGIN
    ...
    LOOP
        ...
        -- 7. Create InventoryVisibility table for role-based access
        EXECUTE format('
        ...
        -- 8. Create InventoryChangeLog for tracking updates
        EXECUTE format('
        ...
        -- 9. Create InventoryAlert table for overbooking notifications
        EXECUTE format('
...
-- Now create functions for each organization
DO $$
DECLARE
    org_schema TEXT;
BEGIN
    FOR org_schema IN 
        ...
        WHERE schema_name LIKE 'org_%'
    LOOP
        -- Create function to calculate spots based on episode length
        EXECUTE format('
        ...
        -- Create trigger function for auto-calculating inventory
        EXECUTE format('
            ...
            BEGIN
                ...
                IF NEW.status = ''scheduled'' AND NEW."airDate" > CURRENT_DATE THEN
                    ...
                    ELSE
                        -- Update existing inventory if no bookings
        ...
        -- Create function to handle inventory holds from orders
        EXECUTE format('
        ...
        -- Add foreign key constraint for Order -> ScheduleBuilder if not exists
        BEGIN
            EXECUTE format('
                ALTER TABLE %I."Order" 
