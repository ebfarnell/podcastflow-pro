-- Schedule Builder Enhanced Schema (Fixed Version)
...
BEGIN
    ...
    -- Rename existing Schedule and ScheduleItem tables if they exist
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = schema_name AND table_name = 'Schedule') THEN
        ALTER TABLE "Schedule" RENAME TO "ScheduleLegacy";
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = schema_name AND table_name = 'ScheduleItem') THEN
        ALTER TABLE "ScheduleItem" RENAME TO "ScheduleItemLegacy";
    END IF;
...
BEGIN
    -- Check if org_podcastflow_pro schema exists
    SELECT EXISTS (
        SELECT 1 FROM information_schema.schemata 
        WHERE schema_name = 'org_podcastflow_pro'
    ) INTO schema_exists;
    ...
    IF schema_exists THEN
        ...
        -- Add show configurations for existing shows only if not already exists
