...
# Script to audit Lambda function invocations for last 7 days
REGION="us-east-1"
DAYS_AGO=7
START_TIME=$(date -d "$DAYS_AGO days ago" +%s)000

echo "Function Name,Last Modified,Last Invocation,Invocation Count (7d),Status"
...
aws lambda list-functions --region $REGION --query "Functions[*].[FunctionName,LastModified]" --output text | while read -r function_name last_modified; do
    ...
    echo "$function_name,$last_modified,$invocation_count,$status"
done | sort -t, -k4,4 -k3,3n