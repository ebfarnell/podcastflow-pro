...
exports.handler = async (event) => {
  ...
  try {
    ...
    switch (httpMethod) {
      case 'GET':
        if (userId) {
          return await getUser(userId, CORS_HEADERS);
        }
        return await listUsers(event.queryStringParameters, CORS_HEADERS);

      case 'POST':
        if (path.includes('/invite')) {
          return await inviteUser(JSON.parse(body), CORS_HEADERS);
        }
        return await createUser(JSON.parse(body), CORS_HEADERS);

      case 'PUT':
        if (userId) {
          if (path.includes('/role')) {
            return await updateUserRole(userId, JSON.parse(body), CORS_HEADERS);
          }
          if (path.includes('/status')) {
            return await updateUserStatus(userId, JSON.parse(body), CORS_HEADERS);
          }
          return await updateUser(userId, JSON.parse(body), CORS_HEADERS);
        }
        break;

      case 'DELETE':
        if (userId) {
          return await deleteUser(userId, CORS_HEADERS);
        }
        break;
    }
...
async function listUsers(queryParams, CORS_HEADERS) {
...
async function getUser(userId, CORS_HEADERS) {
  const params = {
    TableName: TABLE_NAME,
    Key: {
      PK: `USER#${userId}`,
...
async function createUser(userData, CORS_HEADERS) {
...
async function updateUser(userId, updateData, CORS_HEADERS) {
  const timestamp = new Date().toISOString();
...
async function updateUserRole(userId, data, CORS_HEADERS) {
  const timestamp = new Date().toISOString();
...
async function updateUserStatus(userId, data, CORS_HEADERS) {
  const timestamp = new Date().toISOString();
...
async function deleteUser(userId, CORS_HEADERS) {
...
async function inviteUser(data, CORS_HEADERS) {
  ...
  return await createUser(userData, CORS_HEADERS);
}
