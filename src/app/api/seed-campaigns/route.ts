...
export async function POST(request: NextRequest) {
  try {
    // Check for seed auth header
    const seedAuth = request.headers.get('x-seed-auth')
    if (seedAuth !== 'seed-campaigns-2025') {
    ...
    if (!org) {
      return NextResponse.json({ error: 'PodcastFlow Pro organization not found' }, { status: 404 })
    }
    ...
    let advertisers = advertisersResult.rows
    ...
    if (!user) {
      return NextResponse.json({ error: 'No sales/admin user found in organization' }, { status: 400 })
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'Campaign seeding complete!',
      statistics: {
        total: parseInt(stats.total),
        active: parseInt(stats.active),
        proposal: parseInt(stats.proposal),
        completed: parseInt(stats.completed),
        pending: parseInt(stats.pending)
      },
      createdCampaigns: createdCampaigns.length,
      sampleCampaigns: createdCampaigns.slice(0, 3)
    })
    
  } catch (error) {
    console.error('❌ Error seeding campaigns:', error)
    return NextResponse.json(
      { error: 'Failed to seed campaigns', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}