/**
 * YouTube Sync Orchestrator with Quota Management
 * 
 * Enhanced version that enforces quota limits during sync operations.
 * Features:
 * - Quota-aware fetching with automatic stopping when limit reached
...
import { quotaManager, QuotaExceededError } from './quota-manager'
...
/**
 * Fetch data from YouTube API with quota management
 */
async function fetchWithQuota(
  url: string,
  organizationId: string,
  endpoint: 'playlistItems.list' | 'videos.list',
  cost: number,
  maxRetries: number = 3
): Promise<any> {
  // Check quota before making the call
  const quotaCheck = await quotaManager.checkQuota(organizationId, endpoint, cost)
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      await quotaManager.recordUsage(organizationId, endpoint, cost)
      
      return data
    } catch (error) {
      lastError = error as Error
      
      if (error instanceof QuotaExceededError || error instanceof YouTubeQuotaError) {
...
/**
 * Sync YouTube channel uploads to Episodes with quota management
 */
export async function syncYouTubeUploadsWithQuota(options: SyncOptions): Promise<SyncResult> {
  const {
    orgSlug,
    showId,
    ...
    try {
      ...
      const syncLogResult = await querySchema(orgSlug, syncLogQuery, [
        ...
        JSON.stringify({
          ...
          quotaEnforced: process.env.YOUTUBE_QUOTA_ENFORCEMENT === 'true'
        })
      ])
  ...
  try {
    ...
    const quotaStatus = await quotaManager.getQuotaStatus(organizationId)
    if (!quotaStatus.canSync) {
      throw new QuotaExceededError(
        quotaStatus.used,
        quotaStatus.limit,
        quotaStatus.resetAt
    ...
    if (!channelId || !uploadsPlaylistId) {
      ...
      // Channel resolution with quota (costs ~5 units)
      try {
        const channelInfo = await quotaManager.executeWithQuota(
          organizationId,
          'channels.list',
          async () => await resolveYouTubeChannel(apiKey, show.youtubeChannelUrl),
          { cost: 5 }
        )
    ...
    // Fetch playlist items (videos) with quota management
    ...
    while (pagesProcessed < maxPages) {
      ...
      try {
        // Cost: 2 parts = 2 units
        const playlistData = await fetchWithQuota(
        ...
        if (error instanceof QuotaExceededError) {
          console.log('Quota exceeded during playlist fetch, stopping sync gracefully')
          result.stoppedDueToQuota = true
    ...
    // Fetch detailed video information in batches with quota management
    ...
    for (let i = 0; i < videoIds.length; i += batchSize) {
      ...
      try {
        // Cost: 3 parts = 3 units
        const detailsData = await fetchWithQuota(
          detailsUrl.toString(),
          organizationId,
          'videos.list',
          3
        ...
        if (error instanceof QuotaExceededError) {
          console.log('Quota exceeded during video details fetch, processing what we have')
          result.stoppedDueToQuota = true
    ...
    result.success = !result.stoppedDueToQuota || result.videosProcessed > 0
    ...
    if (syncLog && !dryRun) {
      ...
      await querySchema(orgSlug, updateSyncLogQuery, [
        result.stoppedDueToQuota ? 'partial' : 'completed',
        ...
        JSON.stringify({
          ...
          stoppedDueToQuota: result.stoppedDueToQuota
        }),
        syncLog.id
      ])
    }
    ...
    if (syncLog && !dryRun) {
      ...
      await querySchema(orgSlug, updateSyncLogErrorQuery, [
        ...
        JSON.stringify({
          ...
          quotaExceeded: error instanceof QuotaExceededError
        }),
        result.quotaUsed,
        JSON.stringify({
          created: result.episodesCreated,
    ...
    if (error instanceof QuotaExceededError) {
      // Don't throw quota errors as failures if we processed some videos
