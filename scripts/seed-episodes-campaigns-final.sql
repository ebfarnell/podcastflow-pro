...
BEGIN
    ...
    LOOP
        ...
        WHILE episode_date <= end_date LOOP
            ...
            INSERT INTO "Episode" (
                id, "showId", "organizationId", title, "episodeNumber", 
                duration, "airDate", status, "createdAt", "updatedAt"
            ) VALUES (
                episode_id,
                show_record.id,
            ...
            INSERT INTO "EpisodeAnalytics" (
                id, "episodeId", "organizationId", date, downloads, "uniqueListeners", 
                completions, "avgListenTime", "spotifyListens", "appleListens", "googleListens", "otherListens",
                shares, likes, comments, "adRevenue", "createdAt", "updatedAt"
            ) VALUES (
                gen_random_uuid(),
                episode_id,
                show_record."organizationId",
                ...
                -- 70-85% of downloads convert to unique listeners
                ((1500 + RANDOM() * 8500) * (0.70 + RANDOM() * 0.15))::INTEGER,
                -- Completions: 40-70% of unique listeners
                ((1500 + RANDOM() * 8500) * (0.70 + RANDOM() * 0.15) * (0.40 + RANDOM() * 0.30))::INTEGER,
                -- Average listen time correlated with episode duration
                episode_duration * (0.40 + RANDOM() * 0.40),
                -- Platform distribution based on industry averages
                ((1500 + RANDOM() * 8500) * 0.33)::INTEGER, -- Spotify: ~33%
                ((1500 + RANDOM() * 8500) * 0.42)::INTEGER, -- Apple: ~42%
                ((1500 + RANDOM() * 8500) * 0.12)::INTEGER, -- Google: ~12%
                ((1500 + RANDOM() * 8500) * 0.13)::INTEGER, -- Other: ~13%
                (RANDOM() * 200)::INTEGER, -- Shares
                (RANDOM() * 500)::INTEGER, -- Likes
                (RANDOM() * 50)::INTEGER,  -- Comments
                (50 + RANDOM() * 450), -- Ad revenue $50-500
                episode_date,
                episode_date
            );
...
BEGIN
    ...
    FOR i IN 1..40 LOOP
        ...
        campaign_start := start_date + (RANDOM() * EXTRACT(EPOCH FROM (end_date - start_date - INTERVAL '60 days'))/86400)::INTEGER * INTERVAL '1 day';
...
BEGIN
    ...
    LOOP
        ...
        WHILE current_day <= end_date LOOP
            ...
            INTO daily_downloads, daily_listeners, daily_completions, rating
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'Episode Date Range: % to %', date_range_start, date_range_end;
    RAISE NOTICE 'All date filters and analytics dashboards should now work properly!';
END $$;